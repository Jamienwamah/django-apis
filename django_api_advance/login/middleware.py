...
class EncryptionMiddleware:
    ...
    def __call__(self, request):
        ...
        if hasattr(response, 'data') and 'encrypted_data' in response.data:
            # Decrypt the encrypted data in the response
            key = settings.SECRET_KEY.encode('utf-8')
            aes_cipher = AESCipher(key)
            decrypted_data = aes_cipher.decrypt(response.data['encrypted_data']['iv'], response.data['encrypted_data']['ct'])
